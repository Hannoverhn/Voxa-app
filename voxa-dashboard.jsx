...
const C = {
  tinta:"#26215C", tintaDk:"#1a1730", tintaMd:"#2e2870",
  coral:"#A32D2D", coralLt:"#c23535", arena:"#FAEEDA", arenaMd:"#F5C6A0",
  glow:"rgba(163,45,45,0.18)", surface:"rgba(255,255,255,0.04)",
  surface2:"rgba(255,255,255,0.07)", border:"rgba(255,255,255,0.08)",
  border2:"rgba(255,255,255,0.14)", text:"#F5F3EF",
  textSub:"rgba(255,255,255,0.55)", textMut:"rgba(255,255,255,0.28)",
};

const NAV_ITEMS = [
  { icon:"⚡", label:{es:"Campañas",en:"Campaigns"}, path:"/app/dashboard" },
  { icon:"🧠", label:{es:"Business Brain",en:"Business Brain"}, path:"/app/brain" },
  { icon:"🎯", label:{es:"Voxa Predict",en:"Voxa Predict"}, path:"/app/predict" },
  { icon:"🚀", label:{es:"Autopilot",en:"Autopilot"}, path:"/app/autopilot" },
  { icon:"🤖", label:{es:"Agentes",en:"Agents"}, path:"/app/agentes" },
  { icon:"🔍", label:{es:"Clone Pro",en:"Clone Pro"}, path:"/app/clone" },
  { icon:"🎨", label:{es:"Creativos",en:"Creatives"}, path:"/app/creativos" },
  { icon:"📅", label:{es:"Calendario",en:"Calendar"}, path:"/app/calendario" },
  { icon:"📈", label:{es:"ROI",en:"ROI"}, path:"/app/roi" },
  { icon:"🏛️", label:{es:"Modo Político",en:"Political Mode"}, path:"/app/politico" },
  { icon:"🏢", label:{es:"Agencia",en:"Agency"}, path:"/app/agency" },
  { icon:"🌐", label:{es:"Intelligence",en:"Intelligence"}, path:"/app/intelligence" },
  { icon:"⚗️", label:{es:"A/B Testing",en:"A/B Testing"}, path:"/app/abtesting" },
  { icon:"🔗", label:{es:"Afiliados",en:"Affiliates"}, path:"/app/afiliados" },
  { icon:"📊", label:{es:"Progreso",en:"Progress"}, path:"/app/progreso" },
];
  ...
  return (
    <div style={{ width:"100%" }}>
      ...
      <div style={{ height:3, background:"rgba(255,255,255,0.08)", borderRadius:2, overflow:"hidden" }}>
        <div style={{ width:`${pct}%`, height:"100%", background:pct>85?"#fcd34d":C.coral, borderRadius:2, transition:"width .6s ease" }} />
      </div>
    </div>
  );
}
...
export default function VoxaDashboard() {
  ...
  return (
    <div style={{ display:"flex", minHeight:"100vh", background:C.tintaDk, fontFamily:"'Segoe UI',system-ui,sans-serif", color:C.text }}>
      ...
      {/* SIDEBAR */}
      <div style={{ width:sidebarOpen?220:64, background:`linear-gradient(180deg,${C.tinta} 0%,${C.tintaDk} 100%)`, borderRight:`1px solid ${C.border}`, display:"flex", flexDirection:"column", transition:"width .25s cubic-bezier(.4,0,.2,1)", flexShrink:0, position:"relative", zIndex:10 }}>

        <div style={{ padding:"20px 16px 16px", borderBottom:`1px solid ${C.border}` }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ width:36, height:36, background:C.coral, borderRadius:10, display:"flex", alignItems:"center", justifyContent:"center", fontWeight:900, fontSize:16, color:"white", flexShrink:0, boxShadow:`0 4px 16px ${C.glow}`, position:"relative" }}>
              V
          ...
          {NAV_ITEMS.map(item => {
            ...
            return (
              <button key={item.path} onClick={() => goTo(item.path)}
                style={{ width:"100%", display:"flex", alignItems:"center", gap:10, padding:sidebarOpen?"9px 12px":"9px 0", justifyContent:sidebarOpen?"flex-start":"center", borderRadius:10, border:"none", marginBottom:2, cursor:"pointer", background:isActive?"rgba(163,45,45,0.18)":"transparent", transition:"all .15s" }}
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
        ...
        <div style={{ height:60, borderBottom:`1px solid ${C.border}`, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 28px", background:"rgba(26,23,48,0.8)", backdropFilter:"blur(20px)", flexShrink:0 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            ...
            <button onClick={() => goTo("/app/notificaciones")} style={{ width:36, height:36, borderRadius:9, background:C.surface, border:`1px solid ${C.border}`, display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", position:"relative" }}>
              🔔
        ...
        <div style={{ flex:1, overflow:"auto", padding:"28px" }}>
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:28, flexWrap:"wrap", gap:14 }}>
            ...
            <button onClick={() => goTo("/app/generar")}
              style={{ display:"flex", alignItems:"center", gap:8, padding:"12px 22px", background:C.coral, border:"none", borderRadius:12, color:"white", fontSize:14, fontWeight:800, cursor:"pointer", boxShadow:`0 4px 20px ${C.glow}`, transition:"all .2s" }}
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))", gap:14, marginBottom:28 }}>
            {STATS.map((s,i) => (
              <div key={i} style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:16, padding:"20px 22px", transition:"all .2s", cursor:"default", position:"relative", overflow:"hidden" }}
            ...
            <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:18, overflow:"hidden" }}>
              <div style={{ padding:"18px 22px", borderBottom:`1px solid ${C.border}`, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                ...
                <div style={{ display:"flex", gap:6 }}>
                  {(lang==="es"?["Todas","Activas","Pausadas"]:["All","Active","Paused"]).map((f,i) => (
              ...
              {CAMPAIGNS.map((c,i) => {
                ...
                return (
                  <div key={c.id} onClick={() => setSelectedCampaign(c)}
                    style={{ padding:"16px 22px", borderBottom:i<CAMPAIGNS.length-1?`1px solid ${C.border}`:"none", cursor:"pointer", transition:"background .15s", background:isSelected?"rgba(163,45,45,0.08)":"transparent", borderLeft:isSelected?`3px solid ${C.coral}`:"3px solid transparent" }}
                  ...
                  >
                    <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        ...
                        <div style={{ marginTop:8 }}><BudgetBar spent={c.spent} budget={c.budget} /></div>
                      </div>
                    </div>
                  </div>
                );
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                ...
                <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:16, padding:"18px" }}>
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                    {[
                      { icon:"🎯", label:lang==="es"?"Ver Voxa Predict":"View Voxa Predict", path:"/app/predict", accent:true },
                      { icon:"⚡", label:lang==="es"?"Crear nueva campaña":"Create new campaign", path:"/app/generar", accent:false },
                      { icon:"🚀", label:lang==="es"?"Activar Autopilot":"Activate Autopilot", path:"/app/autopilot", accent:false },
                      { icon:"📊", label:lang==="es"?"Ver ROI":"View ROI", path:"/app/roi", accent:false },
                    ].map((a,i) => (
                      <button key={i} onClick={() => goTo(a.path)}
                        style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 14px", borderRadius:10, border:a.accent?`1px solid rgba(163,45,45,0.3)`:`1px solid ${C.border}`, background:a.accent?"rgba(163,45,45,0.1)":C.surface, color:a.accent?C.arena:C.textSub, fontSize:13, fontWeight:a.accent?700:400, cursor:"pointer", textAlign:"left", width:"100%", transition:"all .15s" }}
                ...
                <div style={{ background:`linear-gradient(135deg,rgba(163,45,45,0.12),rgba(38,33,92,0.4))`, border:`1px solid rgba(163,45,45,0.2)`, borderRadius:16, padding:"16px 18px" }}>
                  ...
                  <p style={{ fontSize:13, color:C.textSub, margin:0, lineHeight:1.6 }}>
                    {lang==="es"
                      ? "Los viernes a las 6PM generan un 34% más de clics para campañas como esta."
                      : "Fridays at 6PM generate 34% more clicks for campaigns like this."}
                  </p>
                </div>
              </div>
            )}
